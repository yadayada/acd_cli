...
def size_nlink_str(node, size_bytes=False):
    ...
    if node.is_file:
        ...
        return nor_fmt % str(cache.num_children(node.id)).rjust(7 if not size_bytes else 11)
    return ''


def file_entry(file, long=False, size_bytes=False) -> str:
    return '[{}] [{}] {}{}{}'.format(
        nor_fmt % file.id,
        color_status(file.status),
        (cache.size_nlink_str(file, size_bytes=size_bytes) + ' ') if long else '',
        (date_str(file.modified) + ' ') if long else '',
        color_path(file.name)
    )

def ls_format(folder_id, folder_path=None, recursive=False,
    ...
    if trash_only:
        folders, files = cache.list_trashed_children(folder_id)
    else:
        folders, files = cache.list_children(folder_id, trashed_children)

    if recursive:
        for file in files:
            yield file_entry(file, long, size_bytes)
    ...
    for folder in folders:
        children = cache.num_children(folder.id)
        if recursive and not is_first and children > 0:
            yield ''
        yield '[{}] [{}] {}{}{}{}'.format(
            nor_fmt % folder.id,
            color_status(folder.status),
            (cache.size_nlink_str(folder, size_bytes=size_bytes) + ' ') if long else '',
        ...
        if recursive:
            for n in cache.ls_format(folder.id,
...
def tree_format(node, path, trash=False, dir_only=False,
    ...
    indent += ' ' * 4
    folders, files = cache.list_children(node.id, trash)
    for folder in folders:
        for line in cache.tree_format(folder, '', trash, dir_only, depth + 1, max_depth):
            yield line
...
def long_id_format(nodes) -> 'Generator[str]':
    for node in nodes:
        path = cache.first_path(node.id)
        yield '[{}] [{}] {}{}'.format(
            nor_fmt % node.id,
...
def path_format(nodes):
    for node in nodes:
        yield cache.first_path(node.id) + node.name
